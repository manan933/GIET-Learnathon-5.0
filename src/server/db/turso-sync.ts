...
export async function syncDatabaseWithTurso(db: Database, uploadsDir: string): Promise<boolean> {
	...
	try {
		...
		if (tursoUserCount > 0) {
			...
			// Pull data from Turso into local SQLite
			db.transaction(() => {
			...
			for (const table of SYNC_TABLES) {
				...
				db.transaction(() => {
					for (const row of res.rows) {
						...
						// If this is an attachment and file is missing on local disk, restore it
...
export function enableAutoTursoReplication(db: Database): void {
	...
	(db as any).prepare = function (sql: string) {
		...
		stmt.run = function (...args: unknown[]) {
			...
			if (getTursoClient() && /^\s*(INSERT|UPDATE|DELETE|REPLACE)/i.test(sql)) {
